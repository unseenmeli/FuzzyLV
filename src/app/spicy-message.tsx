...
export default function SpicyMessage() {
  ...
  const messages = React.useMemo(() => {
    ...
    return messageData.messages
      .filter((msg: any) => {
        const msgUsernames = [msg.senderUsername, msg.receiverUsername].sort();
        return msgUsernames[0] === usernames[0] &&
               msgUsernames[1] === usernames[1] &&
               msg.messageType === 'spicy';
      })
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (replyingTo) {
        messageContent.replyTo = replyingTo.id;
        messageContent.replyContent = replyingTo.content || "Image";
      }
    ...
    return (
      <View key={msg.id}>
        ...
        >
          ...
          >
            {msg.replyTo && (
              <View className="bg-black/20 rounded-lg p-2 mb-1 border-l-2 border-red-400">
                <Text className="text-pink-300/60 text-xs">
                  Replying to
                </Text>
                <Text className="text-pink-200 text-sm" numberOfLines={1}>
                  {messages.find((m: any) => m.id === msg.replyTo)?.content || "Message"}
                </Text>
              </View>
            )}
            ...
            {msg.content ? (
              <Text className={isOwnMessage ? "text-red-100" : "text-red-200"}>
                {msg.content}
              </Text>
            ) : null}

            <Text className={isOwnMessage ? "text-red-400/50 text-xs mt-1" : "text-red-500/50 text-xs mt-1"}>
              {new Date(msg.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
            </Text>

            {msg.reaction && (
              <View className="absolute -bottom-2 -right-2 bg-red-900/90 rounded-full px-2 py-1 border border-red-500/50">
                <Text className="text-lg">{msg.reaction}</Text>
              </View>
            )}
          </View>
        </Pressable>
